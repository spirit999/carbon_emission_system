...
import {
  extractSpeciesCarbonList,
  rowToSpeciesAmount,
  rowToSpeciesName
} from "../../utils/speciesCarbon";
export default {
    ...
    getPie() {
      request.get("/api/carbonEmission/listSpeciesCarbon").then((res) => {
        if (res.data.code == 200) {
          ...
          const dataList = extractSpeciesCarbonList(payload);
          if (payload && payload.actualYear != null) {
            this.curYear = parseInt(payload.actualYear, 10);
          }
          this.xValue = [];
          this.yValue = [];
          for (let i = 0; i < dataList.length; i++) {
            const row = dataList[i];
            const name = rowToSpeciesName(row);
            const val = rowToSpeciesAmount(row);
            this.xValue.push(name);
            this.yValue.push({ value: val, name: name });
          }
          this.$nextTick(() => {
            if (this.$refs.assessmentGraph) {
    ...
    getCarbonBar() {
      request.get("/api/carbonEmission/listSpeciesCarbon").then((res) => {
        if (res.data.code == 200) {
          ...
          for (let i = 0; i < dataList.length; i++) {
            const row = dataList[i];
            xValueTemp.push(rowToSpeciesName(row));
            yValueTemp.push(rowToSpeciesAmount(row));
          }
